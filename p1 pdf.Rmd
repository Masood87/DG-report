...
This part provides descriptive analysis of districts and district governors' characteristics. In the first section of Part 1, characteristics of districts are studied both in terms of 4-year trendline as well as with respect to other characteristics of districts and district governors. In the second section of Part 1, characteristics of district governors are studied both across time and with respect to other characteristics of districts and district governors.
...
```{r highwayMap, echo=FALSE, out.width="100%", fig.align="center", fig.cap="Districts that access highway"}
...
Governors of districts with highway access are more likely to be appointed through a competitive process. The trend also indicates growing competitive appointments in the districts with access to highways. In districts without access to highways, there is a slight uptick in uncompetitive appointments in the past one year.
...
unique(df[, c("province", "district", "secure", "border")]) %>% select(3:4) %>% table() %>% data.frame %>% 
  ...
  theme(axis.text.x = element_text(size = 8)) + labs(x="", fill="", y = "Ratio")
```
...
unique(df[, c("province", "district", "secure", "highway")]) %>% select(3:4) %>% table %>% data.frame %>% 
  ...
  theme(axis.text.x = element_text(size = 8, angle = 90, hjust = 1, vjust = .5)) + labs(x="", fill="", y = "Ratio")
```
...
Insecure district on average had more governors that were appointed through an uncompetitive process than secure districts. However, there is a slight uptick in the share of uncompetitively hired governors in secure districts in the past one year. This trend corresponds with an overall growth in uncompetitive recruitments of district governors, which is discussed in Section Appointment / Selection of District Governors.
...
```{r echo=FALSE, out.width="100%", fig.cap="Map of peri-urban districts"}
include_graphics("~/Documents/My Consultancies/IDLG District Governor's Report/Maps/periurban.jpg")
```
...
There is a strong relationship between how large opium poppy cultivation is in a district and the level of (in)security in that district. A simple correlation between security and opium poppy cultivation yields a coefficient of `r secOpiumCorr`%, which is considered a very strong negative relationship. Furthermore, as the following graph visualizes, the collapsed districts account for the largest area of poppy cultivation in the country on average, which could indicate a causal relationship between security and opium poppy cultivation.

```{r echo=FALSE, fig.cap="Average opium poppy cultivation by security status"}
...
```{r echo=FALSE, fig.cap="Average opium poppy cultivation by access to highway"}
filter(df2, date %in% c(as.Date("1396-12-30"), as.Date("1395-12-30"), as.Date("1394-12-30"), as.Date("1393-12-30"))) %>% 
...
```{r echo=FALSE, fig.cap="Average opium poppy cultivation by mineral resources (mines)"}
filter(df2, date %in% c(as.Date("1396-12-30"), as.Date("1395-12-30"), as.Date("1394-12-30"), as.Date("1393-12-30"))) %>% 
...
```{r echo=FALSE, fig.cap="Average opium poppy cultivation by district grade"}
filter(df2, date %in% c(as.Date("1396-12-30"), as.Date("1395-12-30"), as.Date("1394-12-30"), as.Date("1393-12-30"))) %>% 
  ...
  labs(x="", y="Average Popply Cultivation in Hectares", fill = "")
```
...
```{r echo=FALSE, fig.cap="Average opium poppy cultivation and peri-urban"}
filter(df2, date %in% c(as.Date("1396-12-30"), as.Date("1395-12-30"), as.Date("1394-12-30"), as.Date("1393-12-30"))) %>% 
