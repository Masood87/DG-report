...
ethnicity96 <- filter(df2, date == as.Date("1396-12-30")) %>% {table(.$majEthnic, useNA = "ifany")} %>% 
  prop.table() %>% {round(.*100, 1)} %>% data.frame() %>% arrange(desc(Freq)) %>% mutate(Freq = format(Freq, nsmall = 1))
majethnic96 <- ethnicity96 %>% filter(Var1 %in% c("Pashtun", "Tajik", "Uzbek", "Hazara")) %>% 
  mutate(Freq = as.numeric(Freq)) %>% {sum(.[[2]])}
```

In the last four years, district governors belonged to 14 different ethnic groups. Majority (`r majethnic96`%) of them belong to 4 major ethnic groups. `r ethnicity96[1,1]` ethnic governors accounts for the largest share of district governors (`r ethnicity96[1,2]`%), followed by `r ethnicity96[2,1]` (`r ethnicity96[2,2]`%), `r ethnicity96[3,1]` (`r ethnicity96[3,2]`%) and `r ethnicity96[4,1]` (`r ethnicity96[4,2]`%). Looking at the 4-year trend, the share of Pashtun and Tajik governors have increased slightly.

```{r echo=FALSE, fig.cap="Change in ethnicity of District governors since inception of NUG"}
filter(df2, date %in% c(nugIncept, as.Date("1396-12-30"))) %>% .[, c("majEthnic", "date")] %>% table(useNA = "ifany") %>% 
  prop.table(2) %>% data.frame %>% filter(!is.na(majEthnic)) %>% ggplot(aes(majEthnic, Freq, fill = date)) + theme_bw() + 
  labs(x="", y="Ratio", fill="") + geom_col(position = "dodge")
```
...
Native district governors tend to have more presence in the pre-urban districts. Another notable difference is districts bordering Pakistan have more non-native governors, while other districts bordering countries other than Pakistan have more native governors than average. The share of native governors is also higher among districts that are classified as highly insecure. Moreover, almost all governors of districts with ports and districts classified as grade 1 district used to be native until mid-1395. Districts with highways are also more likely to have native governors. The relationship between security and whether district governor is native does not appear to be strong--figure below visualizes that.
...
Native district governors are on average older (`r nativeAge[2,2]` years) than non-native district governors (`r nativeAge[1,2]`. Non-native governors, being on average younger, also have a higher level of education on average, and are appointed more likely via a competitive process.
...
The following graph provides a visual presentation of number of new district government appointments across time. The red line indicates the inception of the NUG.
